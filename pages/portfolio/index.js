...
const PortfolioPage = ({ category, categoryImages }) => {
  ...
  return (
    <>
      {loading ? (
        <PortfolioTitlePageSkeleton />
      ) : (
        <>
          <Head>
            <title>Among creatives | portfolio</title>
          </Head>
          <div className="px-4 mt-24 lg:mt-4 lg:px-20 -z-50">
            ...
            <div className="mt-10 max-w-3xl lg:pl-20">
              <p>
                ...
                from fashion to products helping them to tell their story
                through my lens.
              </p>
...
export async function getStaticProps() {
  const [categoryResponse, imagesResponse] = await Promise.all([
    ...
    fetcher(
      `${process.env.NEXT_PUBLIC_STRAPI_URL}/portfolio-images?populate=*`
    ),
  ]);
